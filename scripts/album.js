...
// Assigns previously static song row template to a variable called template and return it.
 ...
 window.onload = function() {
   ...
   var findParentByClassName = function(element, targetClass) {
     if (element) {
       //sets currentParent to the elements parentElement
       var currentParent = element.parentElement;
       // Checks to see if currentParent equals target class or null if not it will keep looping
       //until the currentParent equals the target class or null
   ...
   for (var i = 0; i < songRows.length; i++) {
     songRows[i].addEventListener('mouseleave', function(event){
       ...
       // checks the item the mouse is leaving is not the current song.
